...
if ( !$domain ) {
    print "Usage: site_info.pl <domain>\n";
    exit(0);
}
...
# my ($ip_domain, $ip_www, $ip_custom) = (0,0,0);


# my $ip_domain = $dns->{'domain'}->{'ip'} if ( exists($dns->{'domain'}->{'ip'}) );
# my $ip_www    = $dns->{'www'}->{'ip'}    if ( exists($dns->{'www'}->{'ip'}) );
# my $ip_custom = $dns->{'custom'}->{'ip'} if ( exists($dns->{'custom'}->{'ip'}) );
# 
# print "\n\n*** DNS information ***\n";
# print "Web:\n";
# if ( $ip_domain ) {
#     print "      $raw_domain: " . $ip_domain . " (" . $dns->{'domain'}->{'org'} . ")\n";
# }
# 
# if ( $ip_www ) {
#     print "  www.$raw_domain: " . $ip_www .    " (" . $dns->{'www'}->{'org'} . ")\n";
# }
# 
# if ( $ip_custom ) {
#     print "  www.$domain: "     . $ip_custom . " (" . $dns->{'custom'}->{'org'} . ")\n";
# }
...
sub check_dns
{
    ...
    # IP v6 seems to throw a warning (on my mini), so force ip v4.
    $res->force_v4(1);
